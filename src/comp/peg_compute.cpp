...
void
PEGCompute::genS_RuleEdges(vertexid_t index, ComputationSet &compset, ContainersToMerge &containers, Grammar *grammar) {
    vertexid_t numEdges = compset.getDeltasNumEdges(
            index);                 //## can we make sure that the deltas is uniqueness
...
void PEGCompute::genD_RuleEdges(vertexid_t index, ComputationSet &compset, ContainersToMerge &containers, bool isOld,
                                Grammar *grammar) {
    vertexid_t numEdges;
    vertexid_t *edges;
    char *labels;
...
//        if (indexInCompset >= 0 && indexInCompset < compset.getNumVertices())
        checkEdges(edges[i], labels[i], compset, containers, isOld, grammar);
    }
}
    ...
    for (auto it = vertexSet.begin(); it != vertexSet.end(); it++) {
        ...
        if (oldEmpty) {
            ...
            if (!deltaEmpty) {
                ...
                myalgo::unionTwoArray(len, edges, labels, n1, compset.getOldsEdges(i), compset.getOldsLabels(i), n2,
