...
//			store_graphs_in(file_graphs_in, cfg, graphstore, it->second);
    ...
    static void do_worklist_ooc_synchronous(CFG *cfg_, GraphStore *graphstore, Grammar *grammar, Singletons *singletons,
        ...
        while (!worklist_1->isEmpty()) {
            ...
            for (unsigned int i = 0; i < NUM_THREADS; i++)
                comp_threads.push_back(std::thread([=] {
                    compute_ooc(cfg, graphstore, worklist_1, worklist_2, grammar, tmp_graphstore, singletons, actives,
                                flag);
                }));
...
//	    dynamic_cast<NaiveGraphStore*>(graphstore)->printOutInfo();
    ...
    static void compute_ooc(CFG_map_outcore *cfg, GraphStore *graphstore, Concurrent_Worklist<CFGNode *> *worklist_1,
                            Concurrent_Worklist<CFGNode *> *worklist_2, Grammar *grammar, GraphStore *tmp_graphstore,
                            Singletons *singletons, Concurrent_Worklist<CFGNode *> *actives, bool flag) {
        CFGNode *cfg_node;
 ...
 * */
            ...
            cout << "open file:" << file_actives << "failed" << endl;
        }
    }


    //append the updated in_mirrors into the corresponding file
    static void store_graphs_in(const string &file_graphs_in, CFG *cfg, GraphStore *graphstore,
...
//					assert(graph != nullptr);
            ...
            if (myfile.is_open()) {
                ...
                for (auto &n : s) {
                    auto pointer = n->getOutPointer();
                    PEGraph *graph = graphstore->retrieve(pointer); // todo add timer
                    if (graph) {
                        //write a pegraph into file
                        out.write(reinterpret_cast<char *>(&pointer), sizeof(pointer));
                        graph->write_unreadable(out);
                    }
                }
                myfile<<out.str();
                myfile.close();
